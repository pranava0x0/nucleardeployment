...
## Deployment Core project map

- `app/data.ts`: canonical MVP records, stage vocabulary, source URLs, and headline counts.
- `app/page.tsx`: national overview.
- `app/deployments/`: directory and project detail template.
- `app/map/`: schematic map plus synchronized list.
- `app/federal-action/`: executive-order and DOE-program crosswalk.
- `app/capital/`: financing and supply-chain view.
- `app/methodology/`: public definitions, inclusion rules, and limitations.
- `app/globals.css`: project design tokens and responsive system.
- `docs/research-foundation.md`: research baseline and expansion contract.
...
For data edits, read the source page itself, copy its exact URL, update `captured_at`/methodology notes when the schema adds them, and inspect every visible count. Do not treat the schematic map as geographic ground truth. No subagent or workflow was used for the initial MVP.
