...
export default function MethodologyPage() {
  return <PageShell><main id="main" className="inner-page methodology-page"><header className="page-lead grid-bg"><p className="eyebrow">Method · Version 0.2 · 2026-07-17</p><h1>Evidence before labels.</h1><p>Every record separates what happened, who reported it, which stage it applies to, and what evidence would move the project forward.</p></header>
    ...
    <section className="section"><div className="section-head"><div><p className="eyebrow">Technology taxonomy · Four separate axes</p><h2>Generation is not size. Size is not reactor family.</h2></div><p>Each project carries generation, scale, reactor family, and operating role. “SMR” describes scale; “PWR” and “BWR” describe light-water families.</p></div><div className="tech-table">{technologies.map((tech) => <div key={tech.name}><b>{tech.name}</b><span>{tech.size}</span><span>{tech.fuel}</span><p>{tech.proof}</p></div>)}</div></section>
