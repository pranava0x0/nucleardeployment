...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: canonicalUrl(`/deployments/${slug}`) },
  };
}
...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const breadcrumbs = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Deployments", item: absoluteUrl("/deployments") },
      { "@type": "ListItem", position: 3, name: project.name, item: absoluteUrl(`/deployments/${project.slug}`) },
    ],
  };
  return <PageShell><JsonLd data={breadcrumbs} /><main id="main" className="inner-page project-page">
