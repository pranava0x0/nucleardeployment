...
export default async function CompanyPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <PageShell><main id="main" className="inner-page company-page">
    ...
    <section className="section">
      <div className="company-summary"><div><span>Technology</span><b>{company.technology}</b></div><div><span>Reactor classes</span><b>{reactorClasses}</b></div><div><span>Tracked projects</span><b>{companyProjects.length}</b></div><div><span>Highest deployment stage</span><b>{highestStage} · {stageLabels[highestStage - 1]}</b></div><div><span>Evidence</span><a href={company.source} target="_blank" rel="noreferrer">{company.sourceLabel} ↗</a></div></div>
