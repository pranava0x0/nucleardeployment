...
- [x] **Medium. Make the whole board row clickable** (UX-002 in issues.md). Done 2026-08-08 with a stretched company link over each row. The dossier "back to the board" link (UX-001) remains open: the dossier back link still points at `/companies`, not the board.
...
- [ ] **Low. Point the company dossier back link at the board** (UX-001) or add a second "back to the board" link beside "All companies", so the scan-open-return loop lands where it started.
