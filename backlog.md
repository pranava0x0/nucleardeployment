...
- [x] **Medium. Lead the homepage with toplines and news, not the full 18-row table.** Done 2026-08-23: added a "Catch up" strip (one terse, sourced line per section: latest news, next gate, federal action, capital instrument) directly under the stat strip, moved "Latest developments" and "Next gates" above the race board, and trimmed the board itself to its top 6 rows by default with a "Show all 18 companies" `<details>` toggle for the rest (all 18 still render; only the DOM disclosure state changes, so nothing is fetched or hidden from crawlers). The board's filter opens a matching row's `<details>` ancestor explicitly, since a closed `<details>` hides its content natively regardless of the `hidden` attribute (DESIGN.md § 12.3's trap, guarded by a source-level test). Measured 8.90 screens at 375×812, down from 11.1 despite the financing and BD layers adding content since that measurement. Touch targets on the new toggle and catch-up links measured 44px under `(pointer: coarse)`.
...
- [ ] **Medium. Revisit whether news/gates belong above the race board, or the board belongs higher.** A 2026-08-23 UX review argued the homepage now stacks five content blocks (hero, stats, catch-up, latest developments, next gates) before the site's core artifact, and a visitor who came to see "how are the 18 companies doing" scrolls a long way first. This is the direct tradeoff the same session's toplines-first reorg was built and approved to make (see the entry above), so it wasn't reversed unilaterally. Worth a second look once there's real traffic data or user feedback, not on review-agent opinion alone.
- [ ] **Low. "BD" nav label is not self-explanatory.** Every sibling tab is a full word or phrase (Federal Action, Capital + Supply, Financing, Methodology); "BD" alone fails a first-time-visitor read, per a 2026-08-23 UX review. The page's own `<h1>` already says "Business development" — consider matching the nav label to it.
- [ ] **Low. Mobile primary nav has no scroll affordance.** At 375px the nav strip scrolls horizontally (contained, no page-level overflow) but gives no visual cue that Federal Action/Capital/Financing/BD/Methodology are one swipe past what's visible. A subtle edge fade or arrow would signal there's more. Found in the same 2026-08-23 UX review.
- [ ] **Low. Consolidate the split mobile/coarse-pointer media-query blocks.** `@media (max-width: 639px)` and `@media (pointer: coarse)` each appear twice in `app/globals.css`, hundreds of lines apart, because BD-specific rules were appended at file-end instead of merged into the existing blocks. Not currently causing a bug, but invites a future contradictory rule landing in the wrong copy. Found in a 2026-08-23 architecture review.
