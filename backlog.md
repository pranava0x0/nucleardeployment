...
- [ ] **High.** Fix the 14 findings from the post-merge review of PR #6, on one follow-up PR. Highest first: `check-links.mjs` classifies any network failure as a dead link and writes it to the append-only history; `--limit` with a bad value checks nothing and exits 0; `npm run data:check` is not in CI, so 129 of 190 sourced records have no automated validation; the executed race bar clips past 1,200 MWe with no overflow marker and the dossier can print "196% of the way there"; `deploy-pages.yml` uses moving action tags while `ci.yml` is SHA-pinned; the llms.txt base URL is hardcoded seven times against `NEXT_PUBLIC_SITE_URL`; llms.txt prints "1 companies".
