...
- [x] Add the financing layer (2026-08-11, `jam/nuclear-financing-analysis-a02636`): `/financing` with cost benchmarks by reactor class (FOAK/NOAK/actual/target), a sourced learning ladder, per-entrant contract readiness (model, government vehicle, commercial position, stated cost, labeled next-gate judgment), 18 in-use contracting mechanisms with executed examples plus 2 proposals, Price-Anderson and NEIL pool records, 7 underwriters, overrun history, and siting-by-class; six reports captured to `data/sources/reports/` with page-stamped quote tests; context tables (`capital`, `federalActions`, `programs`) brought under source validation. Record in `docs/financing-analysis-record.md`.
