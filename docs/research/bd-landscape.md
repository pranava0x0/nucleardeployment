# BD landscape: who buys new nuclear, by reactor class (2026-08-17)

Research basis for the BD tab (`app/bd-data.ts`, `/bd`). Facts gathered
2026-08-17 by inline web search; each line carries the URL that supports
exactly that claim and a source-tier tag. Records already in `app/data.ts` or
`app/financing-data.ts` are referenced, not re-researched; this file holds only
what the BD layer adds. The 2026-08-05 pack caution applies here too: verify a
line against its source before re-citing it elsewhere.

## Hyperscalers and data-center operators

- Microsoft signed a 20-year PPA with Constellation for the Crane Clean Energy
  Center (Three Mile Island Unit 1 restart), about 835 MW, announced
  2024-09-20. [company]
  <https://www.constellationenergy.com/news/2024/Constellation-to-Launch-Crane-Clean-Energy-Center-Restoring-Jobs-and-Carbon-Free-Power-to-The-Grid.html>
- Meta signed a 20-year PPA with Constellation for the Clinton Clean Energy
  Center in Illinois, 1,121 MW, announced 2025-06-03, deliveries from June
  2027, supporting relicensing plus a 30 MW uprate. [company]
  <https://www.constellationenergy.com/news/2025/constellation-meta-sign-20-year-deal-for-clean-reliable-nuclear-energy-in-illinois.html>
- Google and NextEra Energy announced the Duane Arnold restart 2025-10-28:
  a 25-year Google PPA on Iowa's only nuclear plant (615 MW), online target
  Q1 2029; Central Iowa Power Cooperative buys the remainder on the same
  terms. [company]
  <https://newsroom.nexteraenergy.com/NextEra-Energy-and-Google-Announce-New-Collaboration-to-Accelerate-Nuclear-Energy-Deployment-in-the-U-S?l=12>
- Amazon's Climate Pledge Fund anchored X-energy's ~$500M Series C-1,
  announced 2024-10-16, funding Xe-100 design completion, licensing, and the
  first TRISO-X fuel facility phase, with a joint target of more than 5 GW
  online in the U.S. by 2039. [company]
  <https://x-energy.com/media/news-releases/amazon-invests-in-x-energy-to-support-advanced-small-modular-nuclear-reactors-and-expand-carbon-free-power>
- The same round gave Amazon two X-energy board seats; the stake size was not
  disclosed. [press]
  <https://www.nucnet.org/news/amazon-buys-stake-in-nuclear-reactor-developer-in-bid-to-power-data-centres-10-4-2024>
- Oracle chairman Larry Ellison told the September 2024 earnings call the
  company is designing a gigawatt-scale data center to be powered by three
  small modular reactors, saying building permits were already secured; no
  vendor, site, or contract has been disclosed since. [press]
  <https://www.cnbc.com/2024/09/10/oracle-is-designing-a-data-center-that-would-be-powered-by-three-small-nuclear-reactors.html>
- Already tracked in the dataset (not re-researched): Amazon-Energy Northwest
  Cascade funded 320 MWe phase; Amazon 5 GW framework; Google-Kairos 500 MW
  master plant development agreement with Hermes 2 under construction;
  Meta-Oklo 1.2 GW Ohio; Meta-TerraPower up to eight plants; Switch-Oklo
  12 GW; Equinix-Oklo $25M prepayment with right of first refusal;
  Equinix-Radiant 20-unit preorder with deposits; Crusoe-Aalo INL
  partnership; NVIDIA-Valar pilot collaboration; Riot-Terrestrial 4 GW
  ceiling; Endeavour-Deep Fission 2 GW; Duke ACE tariff MOUs with Amazon,
  Google, Microsoft, and Nucor.

## Frontier AI labs

- Sam Altman stepped down as Oklo chairman 2025-04-22, a role held since
  2015, explicitly so Oklo could negotiate supply deals with OpenAI and other
  AI companies without a conflict; Altman had taken Oklo public through his
  AltC SPAC in May 2024. Jacob DeWitte became chairman. [press]
  <https://www.cnbc.com/2025/04/22/sam-altman-steps-down-as-oklo-chair-freeing-nuclear-company-up-to-work-with-more-ai-companies.html>
- No frontier lab has an executed nuclear offtake of its own on record as of
  2026-08-17; lab compute sits inside hyperscaler and neocloud facilities,
  so lab demand reaches reactors through the landlords (research finding
  from the absence of any executed record, not a sourced claim).

## Oil and gas

- Liberty Energy CEO Chris Wright personally sat on Oklo's board and was
  nominated Secretary of Energy in November 2024; the only Liberty Energy
  corporate investment the source names is in geothermal company Fervo, not
  Oklo. A PR-review bot on the first BD-layer PR (2026-08-19) caught the
  shipped record conflating Wright's personal board seat with a Liberty
  Energy corporate position; the buyer row was removed rather than
  retitled, since no genuine Liberty Energy-Oklo instrument exists to ship.
  [press]
  <https://techcrunch.com/2024/11/18/trumps-pro-fracking-energy-secretary-pick-has-also-invested-in-geothermal-and-nuclear-startups>
- Alberta committed C$7M to a Cenovus-led C$26.7M feasibility study of SMRs
  for oil-sands steam (steam-assisted gravity drainage), announced
  September 2023; the Pathways Alliance (Canadian Natural, Cenovus,
  ConocoPhillips Canada, Imperial, MEG, Suncor) has evaluated SMRs for
  oil-sands decarbonization. No oil-sands SMR project has advanced to
  development. [press]
  <https://www.world-nuclear-news.org/Articles/Alberta-funds-SMR-deployment-study>
- ExxonMobil is designing a roughly 1.5 GW gas-fired plant with carbon
  capture to serve data centers, per CEO Darren Woods; rival supply to
  nuclear, not nuclear demand. [press]
  <https://www.powermag.com/exxonmobil-planning-large-gas-fired-plant-to-serve-data-centers/>
- Chevron partnered with Engine No. 1 and GE Vernova to develop up to 4 GW
  of gas plants co-located with data centers, with seven turbines ordered
  for 2026 delivery; same rival-supply posture. [press]
  <https://jpt.spe.org/chevron-partners-with-ge-vernova-and-engine-no-1-to-power-up-ai-data-centers-in-us>
- Already tracked: Oklo-Diamondback 50 MW letter of intent for Permian
  operations.

## Federal and defense

- DOE selected four federal sites in December 2025 to host AI data centers
  paired with new energy projects, including co-located advanced reactors:
  Idaho National Laboratory, Oak Ridge Reservation, Paducah, and Savannah
  River. [gov]
  <https://www.energy.gov/articles/doe-announces-site-selection-ai-data-center-and-energy-infrastructure-development-federal>
- The April 2025 request for information that started that program
  contemplated data-center operations beginning by the end of 2027.
  [verified]
  <https://www.federalregister.gov/documents/2025/04/07/2025-05936/request-for-information-on-artificial-intelligence-infrastructure-on-doe-lands>
- DOE's UPRISE initiative (announced 2026-03) targets 2.5 GW of added
  capacity by 2027 and 5 GW by 2029 through uprates, license renewals, and
  restarts; deputy assistant secretary Rian Bahran called uprates and
  restarts the fastest pathway to getting gigawatts on the grid. [press]
  <https://www.powermag.com/doe-unveils-initiative-to-add-5-gw-of-nuclear-capacity-through-uprates-and-restarts/>
- Michael Goff, principal deputy assistant secretary, DOE Office of Nuclear
  Energy: demonstration microreactors are going from start to operation in
  under a year; the administration's goal is 400 GW of nuclear capacity by
  2050; DOE awarded $2.7B for uranium enrichment in January 2026. [press]
  <https://www.ans.org/news/article-8210/nuclear-is-ready-now/>
- Standard Nuclear (NYSE: STDN, TRISO fuel maker, IPO July 2026) appointed
  Seth Cohen, former DOE Chief Counsel for Nuclear Policy, to its board
  effective 2026-08-12; at DOE he coordinated nuclear deployment policy with
  the White House, NRC, and NASA. [verified]
  <https://www.sec.gov/Archives/edgar/data/0002086716/000162828026056936/cohen_boardxappointmentxpr.htm>
- The Air Force finalized ANPI vendor-to-base assignments 2026-04-22: Radiant
  (Kaleidos, 1 MWe) to Buckley Space Force Base, CO; Westinghouse (eVinci) to
  Malmstrom AFB, MT; Antares (R1, 200-300 kW) to Joint Base San Antonio, TX;
  Radiant and Antares target 2028 delivery. [press]
  <https://www.powermag.com/air-force-anpi-picks-put-radiant-antares-westinghouse-on-track-for-first-on‑base-microreactors-by-2028/>
- The Army named nine Janus candidate installations 2025-11-20 after on-site
  assessment: Fort Benning GA, Fort Bragg NC, Fort Campbell KY, Fort Drum NY,
  Fort Hood TX, Fort Wainwright AK, Holston Army Ammunition Plant TN, Joint
  Base Lewis-McChord WA, Redstone Arsenal AL. Fort Wainwright is a net power
  importer since its coal cogeneration plant stopped exporting. DIU opened an
  area-of-interest solicitation the same week; final site count and vendor
  award are still undetermined. [press]
  <https://www.world-nuclear-news.org/articles/us-army-names-potential-microreactor-sites>
- Already tracked: Project Pele ($300M ceiling, BWXT); Air Force/DLA Eielson
  notice of intent (Oklo, 5 MW, 30-year fixed price); Army Janus Program
  solicitation (2025-10); EO 14299 (Army-regulated reactor on an installation
  by 2028-09-30); NANO AFWERX study at Joint Base Anacostia-Bolling; Antares
  NASA Marshall prototype; DOE Reactor Pilot Program, DOME, and Launch Pad.

## States and power authorities

- Governor Hochul directed NYPA in June 2025 to develop at least 1 GW of
  advanced nuclear upstate; NYPA's 2025 solicitations drew responses from
  more than 30 entities (23 developer/partner responses, 8 communities), and
  a developer RFQ followed 2026-01-07. [gov]
  <https://www.nypa.gov/News/Press-Releases/2026/20260623-nuclear>
- The Texas Advanced Nuclear Energy Office opened the $350M Texas Advanced
  Nuclear Development Fund on 2026-04-01: development awards capped at
  $12.5M, construction awards at $120M per project, and a completion bonus
  program that the 2026-27 budget left unfunded. [press]
  <https://www.ans.org/news/2026-04-09/article-7920/texas-opens-350m-in-nuclear-funding/>
- Alaska Governor Dunleavy frames microreactors as the replacement for
  rural diesel generation, with a stated goal of statewide access to
  10-cent power by 2030. [gov]
  <https://gov.alaska.gov/microreactor-regulations-put-alaskan-communities-at-forefront-of-energy-innovation/>
- Already tracked: TVA Clinch River (NRC review, $400M Tier 1 award);
  ENTRA1-TVA 6 GW collaboration; PacifiCorp Natrium selections; Duke Belews
  Creek early site permit; Constellation New York and NPPD Nebraska DOE
  siting awards.

## International markets

- The World Bank Group ended its decades-long exclusion of nuclear power in
  June 2025 and formalized a partnership with the IAEA on 2025-06-26 to
  support nuclear for developing economies, including SMR feasibility
  work. [institution]
  <https://www.worldbank.org/en/news/press-release/2025/06/26/world-bank-group-iaea-formalize-partnership-to-collaborate-on-nuclear-energy-for-development>
- Great British Energy - Nuclear selected Rolls-Royce SMR as preferred
  bidder in June 2025 and signed the development contract, with the first
  three UK SMRs sited at Wylfa and grid connection aimed at the mid-2030s.
  [gov]
  <https://www.gov.uk/government/news/great-british-energy-nuclear-and-rolls-royce-smr-sign-contract>
- GE Vernova Hitachi and Orlen Synthos Green Energy signed the Poland
  Generic Design Agreement for the BWRX-300 on 2026-02-24 in Washington;
  Poland's first SMR site is Włocławek, one of six shortlisted BWRX-300
  locations. [company]
  <https://www.gevernova.com/news/press-releases/deployment-bwrx-300-small-modular-reactor-poland-major-step-forward-design-development>
- Nuclear Power Ghana and Regnum Technology Group agreed in August 2024 to
  deploy a NuScale VOYGR-12 plant, planned as Africa's first commercial
  advanced light-water SMR, with a subsidiary company to own and operate
  it. [press]
  <https://www.nucnet.org/news/african-nation-signs-agreement-for-nuscale-12-module-nuclear-power-plant-8-5-2024>
- Meralco PowerGen and Ultra Safe Nuclear Corporation signed a cooperation
  agreement 2023-11-16 at the APEC Leaders' Summit, witnessed by President
  Marcos, for a four-month pre-feasibility study of USNC's 15 MWe / 45 MWt
  Micro-Modular Reactor (TRISO fuel, HTGR, 40-year licensed life); Meralco
  holds an option to pursue a full feasibility study on the results. [press]
  <https://world-nuclear-news.org/Articles/Philippines-considers-deploying-USNC-microreactors>
- The U.S.-Philippines 123 Agreement (civil nuclear cooperation) was
  formalized in November 2023 and took effect July 2024. USTDA committed
  $2.7M to Meralco PowerGen in February 2026 for a technical review of U.S.
  SMR designs and an implementation roadmap, alongside an SMR control-room
  simulator program and a university nuclear-engineering curriculum
  partnership; the Philippines' stated target is 1,200 MW of nuclear online
  by 2032, 2,400 MW by 2035, 4,800 MW by 2050. [press]
  <https://www.world-nuclear-news.org/articles/us-touts-smr-designs-to-the-philippines>
- Already tracked: U.S.-Japan framework (up to $40B, BWRX-300, Tennessee and
  Alabama); OPG Darlington four-unit program (C$20.9B approved); RoPower
  Romania final investment decision (462 MWe); Saskatchewan Research Council
  as first commercial eVinci customer (pilot by 2029); Last Energy's 34
  executed PPAs in Poland and the UK (680 MW); Holtec-EDF Cottam proposal;
  Sizewell C regulated-asset-base close; BWXT-Tata Chemicals LOI (a
  Mumbai-headquartered buyer for a Wyoming site).

## Remote and high-reliability operations

- Rural Alaska villages pay upward of 55 cents/kWh for diesel-generated
  power; Alaska's statewide average is about 28 cents. [press]
  <https://spectrum.ieee.org/microreactor>
- The same IEEE Spectrum survey reports an all-in cost for the first few
  commercial eVinci units of about 25 cents/kWh, and a Bruce
  Power/Westinghouse study of remote Canadian mines that found an eVinci
  could cut power costs up to 44% versus diesel depending on oil prices and
  carbon fees. [press]
  <https://spectrum.ieee.org/microreactor>
- MIT CEEPR's working paper on Alaskan communities analyzes microreactors
  supplying both heat and electricity as the value case against diesel.
  [institution]
  <https://ceepr.mit.edu/workingpaper/the-value-of-nuclear-microreactors-in-providing-heat-and-electricity-to-alaskan-communities/>
- DOE's Idaho National Laboratory funded and reviewed the Nuclear Alternative
  Project's preliminary feasibility study for Puerto Rico (started 2019-10,
  released 2020-05): SMRs/microreactors could supply zero-emission,
  hurricane-resilient baseload cost-competitive with PREPA's proposed mobile
  gas and combined-cycle fleet replacement. [press]
  <https://www.world-nuclear-news.org/Articles/Small-reactors-feasible-in-Puerto-Rico,-study-conc>
- Puerto Rico's Proyecto de la Cámara 1092 (pending as of a 2026-05-22 House
  Economic Development Commission hearing) would amend the island's energy
  policy act to direct evaluation of SMR deployment; the Puerto Rico Energy
  Bureau's president testified in support. [press]
  <https://www.ans.org/news/article-8091/ansuprm-student-section-advocates-for-puerto-rican-nuclear/>
- Guam: senators say DOD has flagged the concept since roughly 2024 and cite
  ~$400M reportedly set aside for two Guam installations (per Sen. Telo
  Taitague), but the Community Defense Liaison Office says the territory's
  government has received no official DOD notification as of the May 2026
  briefing. No confirmed program; shipped as a stated-interest position, not
  a program. [press]
  <https://www.postguam.com/news/local/lawmakers-want-answers-about-micro-nuclear-reactors-on-guam/article_88cb2824-85aa-4c5c-8dd7-e6ac6cdd2b76.html>
- Already tracked: NEI's 2019 estimate that FOAK microreactors generate at
  $0.14-0.41/kWh, overlapping remote diesel; the Eielson AFB arrangement as
  the defense-remote template.

## Remote-location and allied-buyer research pass (2026-08-17, agent fan-out)

Three parallel research agents (US territories; Indo-Pacific allies; European
and remote-base allies) plus follow-up verification found new, currently-live
positions. Every claim below was independently re-fetched from a primary or
press source before shipping; nothing shipped from an agent's search-summary
reconstruction alone.

- Puerto Rico's C.P. 1092 passed the full House 2026-06-15 and moved to the
  Senate; verified directly against metro.pr (not just the agent's summary).
  <https://www.metro.pr/noticias/2026/06/15/camara-aprueba-medida-que-incorpora-la-energia-nuclear-a-la-politica-publica-energetica-de-puerto-rico/>
- Guam: the FY2026 NDAA (signed 2025-12-21) requires DOD to give Congress and
  Guam's governor at least one year's notice before placing any nuclear
  reactor on the island. One agent-supplied search summary said "180 days";
  a direct fetch of postguam.com's own June 2026 follow-up article settled
  it as "at least one year," which is what shipped. [press]
  <https://www.postguam.com/news/local/congress-nuclear-reactor-push-alarms-locals/article_a63b9661-5cca-43a3-9fae-e06ab5682477.html>
- South Korea: SK Innovation (TerraPower's second-largest shareholder since
  a 2023 equity round) and TerraPower signed an August 2026 term sheet
  toward Korea's first commercial Natrium plant, verified against WNN after
  the agent's two primary-source links (UPI, mbiz.heraldcorp.com) both
  403'd. [press] <https://www.world-nuclear-news.org/articles/terrapower-expands-cooperation-with-korean-partners>
  Also shipped: Nano Nuclear-DS Dansuk non-binding MOU for the KRONOS MMR,
  2026-01-13, verified directly against barchart.com. [press]
  <https://www.barchart.com/story/news/37017636/nano-nuclear-energy-signs-mou-with-ds-dansuk-to-advance-micro-modular-reactor-deployment-in-south-korea>
- Nordic/Baltic/Balkan BWRX-300 pipeline, all verified against company or
  WNN primary text: Vattenfall's Värö Peninsula down-select to GE Vernova
  and Rolls-Royce SMR finalists (2025-08); GE Vernova Hitachi-Fortum
  early-works agreement covering Finland and Sweden (2025-07); Norsk
  Kjernekraft-Aecon cooperation across ten Norwegian sites (2025-11);
  Altra-SGE-GE Vernova Hitachi MOU to assess Lithuania (exact signing date
  unconfirmed by any source this session could fetch directly, so the
  shipped record carries `date: null`); USTDA-BEH Bulgaria prefeasibility
  grant (2024-12); Westinghouse-JAVYS Slovakia AP1000/AP300 MOU (2023-07).
  <https://group.vattenfall.com/press-and-media/newsroom/2025/vattenfall-selects-suppliers-on-the-journey-towards-new-nuclear-power/>
  <https://www.gevernova.com/news/press-releases/ge-vernova-hitachi-nuclear-energy-fortum-agreement-deployment-bwrx-300-small-modular-reactor-finland-sweden>
  <https://www.world-nuclear-news.org/articles/aecon-and-norsk-kjernekraft-to-promote-smrs-in-norway>
  <https://www.neimagazine.com/news/lithuania-considers-smr-technology/>
  <https://www.world-nuclear-news.org/articles/us-grant-agreements-to-support-bulgarian-nuclear-projects>
  <https://info.westinghousenuclear.com/news/westinghouse-ap300-smr-ap1000-interest-in-europe-slovakia-deployment-discussions>
- Not shipped, checked and killed: US Virgin Islands and American Samoa have
  no nuclear/SMR activity on record (confirmed dead ends). Taiwan, Australia
  (civil nuclear is statutorily banned; AUKUS is submarine propulsion, out
  of scope), the COFA states (Palau/Marshall Islands/FSM, whose real energy
  transition is solar-battery microgrids, not nuclear), New Zealand,
  Thailand, and Singapore all returned policy statements or third-country
  vendor ties with no US-vendor buyer position to ship. Falkland Islands,
  Gibraltar, Iceland, and Gulf-state remote/military bases (Al Udeid,
  Bahrain) returned zero nuclear content. Czech Republic's Rolls-Royce SMR
  deal is real but the vendor is UK, not US, so it falls outside this page's
  criterion.
- Not shipped, needs a verified primary read before it can ship: CNMI Senate
  Joint Resolution 24-05 (expressing support for studying SMR/microreactor
  deployment on Saipan, Tinian, and Rota, explicitly tied to the Army's
  Project Janus and EO 14299). Two independent search passes and a second
  session's direct-fetch attempts both hit 403 on every candidate source
  (nminewsservice.com twice, cnmileg.net's own agenda PDF did not carry the
  resolution text). The resolution's existence is corroborated by multiple
  independent mentions, but no session has read primary text directly.
  Backlogged, not fabricated from snippets.

## Manufacturing rate: the path to 10-20 microreactors a year

- Radiant is building its first factory on the Oak Ridge Manhattan Project
  site: construction from early 2026, first mass-produced Kaleidos targeted
  2028, scaling toward 50 reactors a year. [company]
  <https://www.radiantnuclear.com/blog/tennessee-factory/>
- Aalo operates a 40,000 sq ft factory headquarters in Austin and describes
  a planned mass-production plant targeting on the order of 100+ reactors
  (about a gigawatt) a year. [company]
  <https://www.aalo.com/factory>
- Already tracked: TRISO-X TX-1 40-year special nuclear material license
  (first commercial HALEU fuel line); BWXT full Pele core delivered; the
  ANPI 36-month delivery deadline; Equinix's 20-unit preorder as the first
  commercial book; INL's FOAK $325/MWh to NOAK ~$120/MWh factory curve.

## Killed or corrected assumptions

- "Crusoe invested in Aalo": not found. The verified record is the 2026-07
  strategic partnership for a Spark data center at INL; Aalo's disclosed
  Series B investors are Valor Equity Partners, NRG Energy, and Hitachi
  Ventures. Kept as partnership, not equity.
- "Google holds equity in Kairos": not found. The Google position is the
  master plant development agreement (offtake), not an investment. NVIDIA's
  verified equity is in TerraPower (NVentures led the $650M round); its
  Valar tie is a pilot collaboration announced with the Sequoia-led
  Series B.
- "Equinix holds Radiant equity": press shorthand says "Equinix-backed";
  the verifiable instruments are the preorder and deposits. Kept as
  deposits.
- Cenovus's oil-sands SMR study: reporting in 2026 says the study concluded
  SMRs are not viable for SAGD steam under current market conditions, but
  the only source found is a partisan blog, so the shipped record states
  the study and its provincial funding without asserting the conclusion.
- Anthropic's pledge to fund grid upgrades and new generation could not be
  pinned to a dated primary source in this pass; no Anthropic row ships.
- The INL feature story quoting Rian Bahran is behind a WAF (403); his
  UPRISE statements are cited to POWER instead.
- Remote-location research pass (2026-08-17): Diego Garcia and Kwajalein
  Atoll both surface in searches as Pentagon microreactor candidates, but the
  only sourced basis is a DARPA program concept from 2012 (killed for lack of
  funding that year) and a 2018 Army report; no live program, contract, or
  solicitation names either site today. Not shipped. Andersen AFB, Guam was
  checked against the Pacific Deterrence Initiative FY2026/FY2027 budget
  documents; those fund missile-defense infrastructure, not reactors, so the
  Guam position ships as a lawmaker/CDLO exchange (postguam.com), not a DOD
  program.

## Corrections from the cached-snapshot audit (2026-08-17)

Every shipped label was checked against its cached copy after `data:cache`.
Eight claims did not survive contact and were trimmed or re-sourced:

- The IEEE Spectrum microreactor snapshot carries none of the figures the
  search layer attributed to it (55¢ villages, ~25¢ eVinci, 44% mine
  savings, Bruce Power study); the page is JS-heavy and the snapshot holds
  only the opening. All Spectrum-based lines were dropped and the remote
  sector now leans on gov.alaska.gov, MIT CEEPR, and the NEI band.
- The gov.uk contract page does not name Wylfa or the mid-2030s; those
  details rode the selection coverage. Label trimmed to what the page
  states: preferred partner June 2025, contract signed, £2.6B allocated.
- The GE Vernova PGDA release does not name Włocławek; the site selection
  is a separate WNN story, now cached and cited as its own position.
- The ANS Texas-fund article names the two reimbursement programs but not
  the per-award caps ($12.5M/$120M, which came from law-firm summaries);
  the caps were dropped.
- The NYPA June 2026 release says the developer RFQ came "last month," not
  January; the label now says 2026 without a month.
- The TechCrunch piece supports Wright's Oklo board seat and Liberty's
  startup investing, but the $10M/2023 figure was Forbes's; dropped.
- The World Bank release describes the IAEA partnership as its "first
  concrete step to reengage with nuclear power in decades," not a ban
  formally "ended in June 2025"; reworded to the release's own frame.
- The Radiant factory claims are cited to WNN's account (cached clean)
  because radiantnuclear.com blocks scripted fetches; the WNN copy carries
  Oak Ridge, early-2026 construction, first unit 2028, and 50/year.
